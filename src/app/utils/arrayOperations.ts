...
export const getRatedMessages = (messageList: BottleMessage[], messageAmount: number, opened: Set<string>) => {
    if (messageAmount > 0 && Number.isInteger(messageAmount) && messageList.length >= messageAmount && opened.size < messageList.length) {
        ...
        const k = findIndices(messageList, indexSet);
